...
class TestInvariants:
    ...
    def test_delta_call_bounds(self, pricer,standard_market, standard_call):
        call_greeks = pricer.greeks(standard_call, standard_market)

        assert 0 <= call_greeks.delta <= 1

    def test_delta_put_bounds(self, pricer,standard_market, standard_put):
        put_greeks = pricer.greeks(standard_put, standard_market)

        assert 0 >= put_greeks.delta >= -1

    def test_gamma_call_bounds(self, pricer,standard_market, standard_call):
        call_greeks = pricer.greeks(standard_call, standard_market)

        assert call_greeks.gamma >= 0

    def test_gamma_put_bounds(self, pricer,standard_market, standard_put):
        put_greeks = pricer.greeks(standard_put, standard_market)

        assert put_greeks.gamma >= 0

    def test_vega_call_bounds(self, pricer,standard_market, standard_call):
        call_greeks = pricer.greeks(standard_call, standard_market)

        assert call_greeks.vega >= 0

    def test_vega_put_bounds(self, pricer,standard_market, standard_put):
        put_greeks = pricer.greeks(standard_put, standard_market)

        assert put_greeks.vega >= 0

    def test_theta_call_bounds(self, pricer,standard_market, standard_call):
        call_greeks = pricer.greeks(standard_call, standard_market)

        assert call_greeks.theta <= 0

    def test_theta_put_bounds(self, pricer,standard_market, standard_put):
        put_greeks = pricer.greeks(standard_put, standard_market)

        assert put_greeks.theta <= 0

    def test_rho_call_bounds(self, pricer,standard_market, standard_call):
        call_greeks = pricer.greeks(standard_call, standard_market)

        assert call_greeks.rho >= 0

    def test_rho_put_bounds(self, pricer,standard_market, standard_put):
